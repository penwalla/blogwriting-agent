...
def research_node(state: State) -> dict:
    ...
    if len(raw_str) > MAX_CHARS:
        print("WARNING: Truncating raw research results")
        raw_str = raw_str[:MAX_CHARS] + '}]'
...
# 🔥 Smaller model for workers (VERY important)
worker_llm = ChatGroq(
    model="llama-3.1-8b-instant",   # safer than 70B
    temperature=0.4,
    max_tokens=900,                # prevents huge output
)

def safe_llm_call(messages, retries=3):
    for attempt in range(retries):
        try:
            return worker_llm.invoke(messages)
        except RateLimitError:
            wait = 1.2 * (attempt + 1)
            print(f"WARNING: Groq rate limit hit. Sleeping {wait:.1f}s...")
            time.sleep(wait)
...
def worker_node(payload: dict) -> dict:
    ...
    # 🔥 Limit evidence count (prevent token explosion)
    evidence = evidence[:8]
    ...
    # 🔥 Hard cap on evidence text length
    MAX_EVIDENCE_CHARS = 2500
    if len(evidence_text) > MAX_EVIDENCE_CHARS:
        print("WARNING: Truncating evidence_text to prevent TPM overflow")
